...
{
	m_board->SetNumColors(nColors);// Сначала задаем количество цветов

	m_board->SetupBoard();// А затем устанавливаем параметры игровой доски
}
...
{
	// Смотрим, есть ли у нас что-то в стеке "Отмена"
	if (m_undo.empty())
		return;
...
{
	// Убеждаемся, что у нас есть возможность совершить отмену действия
	return !m_undo.empty();
}

void CSameGameDoc::RedoLast()
{
	// Смотрим, есть ли у нас что-то в стеке "Повтор"
	if (m_redo.empty())
		return;
